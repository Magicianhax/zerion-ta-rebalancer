...
async function main() {
  ...
  if (config.telegramBotToken && config.telegramAuthorizedUserIds.length === 0) {
    stdout.write("  4. Add your Telegram user ID to TELEGRAM_AUTHORIZED_USER_IDS in .env\n");
    stdout.write("     (find it via @userinfobot on Telegram), then restart\n");
  }
  stdout.write("  • See docs/RECOVERY.md for restoring the wallet later\n\n");
}
